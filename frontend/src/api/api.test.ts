...
it('deletes a task', async () => {
  // 1) Mock: suppression réussie
  (globalThis as any).fetch = vi.fn(() =>
    Promise.resolve({ ok: true, status: 204 })
  );

  // 2) Act: appel de l’API
  await api.deleteTask(1);

  // 3) Assert: bonne URL + bonne méthode
  ...
  it('updates a task', async () => {
    // 1) Mock: mise à jour réussie --> "Si quelqu’un appelle fetch(), je renvoie une réponse HTTP OK qui contient une tâche mise à jour."
    (globalThis as any).fetch = vi.fn(() =>
      Promise.resolve({
        ok: true,
        json: () =>
          Promise.resolve({ id: 1, title: 'Updated Title', status: 'todo' }),
      })
    );
    ...
    // 3) Assert: bonne URL, bonne méthode et bon body
    expect(globalThis.fetch).toHaveBeenCalledTimes(1); //Cette asser­tion vérifie qu’on a appelé fetch une seule fois pendant le test.
    expect(globalThis.fetch).toHaveBeenCalledWith(
      expect.stringMatching(/\/tasks\/1$/),
    ...
    // Et on vérifie la réponse renvoyée
